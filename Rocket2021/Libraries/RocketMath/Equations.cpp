...
double trajectory::accel(double v_burn, double m, double k, double t)
{
    double v = vel(v_burn, m, k, t);
    return -g - (k/m)*v*v;
}

double trajectory::vel(double v_burn, double m, double k, double t) // 0.12 μs on average
{
	return sqrt(m*g/k)*tan(atan(v_burn*sqrt(k/(m*g)))-t*sqrt(g*k/m));
}

double trajectory::alt(double y_burn, double v_burn, double m, double k, double t) // 0.30 μs on average
{
...
double trajectory::t_a(double v_burn, double m, double k) // 0.10 μs on average
{
